...
func _physics_process(delta: float) -> void:
	...
	# A glass about to run out shakes the view for as long as it lasts. Squared
	# like the trauma, so it only arrives at the very end, and kept out of
	# `_trauma` itself, which is meant to decay.
