...
Always start from the main checkout at `/Users/pierclgr/sandbound`,
never from another worktree:

```bash
cd /Users/pierclgr/sandbound && git fetch origin && git checkout main && git pull --ff-only
```
...
`.claude/` is gitignored, so the worktrees never show up in a diff. The git
root and the Godot project root are the same folder, so a worktree is a
complete project on its own.

Then import the fresh worktree, once:

```bash
godot --headless --import --path .claude/worktrees/<feature>
```
...
Work only inside `.claude/worktrees/<feature>`. Commit as
...
```bash
godot --path /Users/pierclgr/sandbound/.claude/worktrees/<feature>
```
...
```bash
cd /Users/pierclgr/sandbound && git checkout main && git pull --ff-only && git merge <feature> && git push
```
...
```bash
cd /Users/pierclgr/sandbound && git worktree remove .claude/worktrees/<feature> && git branch -d <feature>
```
...
```bash
godot --headless --path <path-to> tests/sand_test.tscn
```
